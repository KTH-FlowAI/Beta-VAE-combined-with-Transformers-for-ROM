...
def make_T_Sequence(cfg,data):
    ...
    nSamples    = (data.shape[1]-seqLen)
    X           = np.empty([nSamples, seqLen, data.shape[-1]])
    Y           = np.empty([nSamples, outLen, data.shape[-1]])
    # Fill the input and output arrays with data
    k = 0
    for i in tqdm(np.arange(data.shape[0])):
        for j in np.arange(data.shape[1]-seqLen- seqLen- cfg.next_step):
            X[k] = data[i, j                 :  j+seqLen]
            Y[k] = data[i, j + cfg.next_step :  j+ outLen + cfg.next_step]
            print(f"X = [{j} to {j+seqLen}]"+\
                  f"Y = [{j+cfg.next_step} to {j + outLen + cfg.next_step}]")
            k    = k + 1
    print(f"The training data has been generated, has shape of {X.shape, Y.shape}")
